      ...
      <div class="table-content">
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
		    ...
		    label="联系电话">
		     <template slot-scope="scope">
                ...
                <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                    ...
                    width="90">
                  <template slot-scope="scope">
                    <el-tag v-if="scope.row.zhuangtai === '禁用'" type="danger" size="small">禁用</el-tag>
            ...
            <el-table-column width="400" :align="contents.tableAlign"
                ...
                <el-button
                  v-if="isAuth('yonghu','修改') && scope.row.zhuangtai !== '禁用'"
                  type="warning"
                  size="mini"
                  @click="toggleStatus(scope.row, '禁用')"
                >禁用</el-button>
                <el-button
                  v-if="isAuth('yonghu','修改') && scope.row.zhuangtai === '禁用'"
                  type="success"
                  size="mini"
                  @click="toggleStatus(scope.row, '正常')"
                >启用</el-button>
...
export default {
  ...
  methods: {
    ...
    // 启用/禁用
    toggleStatus(row, zhuangtai) {
      const action = zhuangtai === '禁用' ? '禁用' : '启用';
      this.$confirm(`确定${action}用户「${row.yonghuzhanghao}」吗?`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.$http({
          url: 'yonghu/updateStatus',
          method: 'post',
          data: { id: row.id, zhuangtai: zhuangtai }
        }).then(({ data }) => {
          if (data && data.code === 0) {
            this.$message({
              message: '操作成功',
